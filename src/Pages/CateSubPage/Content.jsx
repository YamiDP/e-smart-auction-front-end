...
export const Content = () => {
  ...
  const onSubmitByColor = async (e) => {
    e.preventDefault();
    setProduct([])
    console.log(e.target.color_id.value)
    await axios.get(API['getProductsByFirstSub']+param, { params: { color_id: e.target.color_id.value } }).then(function (response) {
  ...
  async function sortByValue (e) {
    ...
    await axios.get(API['getProductsByFirstSub']+param, { params: { sort: paramSort } }).then(function (response) {
      const dataPro = response.data;
      const dataProArr = [];
      Object.keys(dataPro).forEach(function(key) {
  ...
  async function sortByRangePrice () {
    setProduct([])
    await axios.get(API['getProductsByFirstSub']+param, { params: { price_from: minRange, price_to: maxRange} }).then(function (response) {
      const dataPro = response.data;
      const dataProArr = [];
      Object.keys(dataPro).forEach(function(key) {
        if(dataPro[key] !== undefined  && key !== 'page_number')
  ...
  async function getProductBySize (size_id) {
    setProduct([])
    await axios.get(API['getProductsByFirstSub']+param, { params: { size_id: size_id } }).then(function (response) {
      const dataPro = response.data;
      const dataProArr = [];
      Object.keys(dataPro).forEach(function(key) {
  ...
  async function paginationPage (numer) {
    setCurrent(numer)
    await axios.get(API['getProductsByFirstSub']+param, { params: { page: numer } }).then(function (response) {
      const dataPro = response.data;
      const dataProArr = [];
      Object.keys(dataPro).forEach(function(key) {
...
useEffect(() => {
  async function fetchData() {
    await axios.get(API['getProductsByFirstSub']+param).then(function (response) {
      const dataPro = response.data;
      const dataProArr = [];
      Object.keys(dataPro).forEach(function(key) {
        if(dataPro[key] !== undefined && key !== 'page_number')
    ...
    await axios.get(API['getProductsByFirstSub'+param]).then(function (responseProductByCate) {
              ...
              >
                ...
                category.map((item, index) => (
                  <>
                    ...
                    <a href={window.location.origin.toString()+"/product-category-first-sub-category/"+item1.slug}>{item1.name}</a>
                          ...
                          item1.secondSubCategories.map((item2, index4) => (
                           <>
                            <li className="cat-item cat-item-97">
                              <a href={window.location.origin.toString()+"/product-category-second-sub-category/"+item2.slug}>
                              {item2.name}
                              </a>
                            </li>
